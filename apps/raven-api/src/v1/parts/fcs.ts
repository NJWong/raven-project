...
router.get('/', async (request: Request, env: Env) => {
  ...
  const result = await db.select().from(fcs).where(eq(fcs.regulationVersion, regulationVersion)).orderBy(fcs.id).limit(cappedLimit).offset(offset)
  const countResult = await db.select({ count: sql<number>`count(*)` }).from(fcs)
...
router.get('/:id', async (request: Request, env: Env) => {
  ...
  const result = await db.select().from(fcs).where(eq(fcs.id, id))
