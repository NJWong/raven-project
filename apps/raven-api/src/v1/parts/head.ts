...
router.get('/', async (request: Request, env: Env) => {
  ...
  const result = await db.select().from(head).where(eq(head.regulationVersion, regulationVersion)).orderBy(head.id).limit(cappedLimit).offset(offset)
  const countResult = await db.select({ count: sql<number>`count(*)` }).from(head)
