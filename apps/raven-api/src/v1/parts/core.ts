...
import { core } from '../../../drizzle/schema'
import { CURRENT_VERSION, REGULATION_VERSIONS } from '../consts/regulationVersion'

const router = Router({ base: '/api/v1/parts/core' })
...
router.get('/', async (request: Request, env: Env) => {
  ...
  const result = await db.select().from(core).where(eq(core.regulationVersion, regulationVersion)).orderBy(core.id).limit(cappedLimit).offset(offset)
  const countResult = await db.select({ count: sql<number>`count(*)` }).from(core)
...
router.get('/:id', async (request: Request, env: Env) => {
  ...
  const result = await db.select().from(core).where(eq(core.id, id))
