...
import { booster } from '../../../drizzle/schema'
import { CURRENT_VERSION, REGULATION_VERSIONS } from '../consts/regulationVersion'

const router = Router({ base: '/api/v1/parts/booster' })
...
router.get('/', async (request: Request, env: Env) => {
  ...
  const result = await db.select().from(booster).where(eq(booster.regulationVersion, regulationVersion)).orderBy(booster.id).limit(cappedLimit).offset(offset)
  const countResult = await db.select({ count: sql<number>`count(*)` }).from(booster)
...
router.get('/:id', async (request: Request, env: Env) => {
  ...
  const result = await db.select().from(booster).where(eq(booster.id, id))
